...
trait StateTransitionRules {
  ...
  object StateTransitionRules {
    ...
    val StateTransitions: Set[StateTransition] = Set(
       DRAFT                      -> DRAFT,
       DRAFT                      -> ARCHIVED                    require UserInfo.PublishRoles illegalStatuses Set(PUBLISHED),
       DRAFT                      -> QUEUED_FOR_LANGUAGE         keepStates Set(PUBLISHED),
       DRAFT                      -> QUEUED_FOR_PUBLISHING       keepStates Set(PUBLISHED) require UserInfo.PublishRoles,
       ARCHIVED                   -> ARCHIVED,
       ARCHIVED                   -> DRAFT,
      (DRAFT                      -> PUBLISHED)                  keepStates Set() require UserInfo.DirectPublishRoles withSideEffect publishConcept,
       QUEUED_FOR_PUBLISHING      -> QUEUED_FOR_PUBLISHING       keepStates Set(PUBLISHED),
      (QUEUED_FOR_PUBLISHING      -> PUBLISHED)                  keepStates Set() require UserInfo.DirectPublishRoles withSideEffect publishConcept,
       QUEUED_FOR_PUBLISHING      -> DRAFT                       keepStates Set(PUBLISHED),
      (PUBLISHED                  -> DRAFT)                      keepCurrentOnTransition,
      (PUBLISHED                  -> UNPUBLISHED)                keepStates Set() require UserInfo.DirectPublishRoles withSideEffect unpublishConcept,
      (UNPUBLISHED                -> PUBLISHED)                  keepStates Set() require UserInfo.DirectPublishRoles withSideEffect publishConcept,
       UNPUBLISHED                -> DRAFT,
       UNPUBLISHED                -> UNPUBLISHED,
       UNPUBLISHED                -> ARCHIVED                    require UserInfo.PublishRoles illegalStatuses Set(PUBLISHED),
      ...
      (TRANSLATED                 -> PUBLISHED)                  keepStates Set() require UserInfo.DirectPublishRoles withSideEffect publishConcept,
    )
    // format: on
